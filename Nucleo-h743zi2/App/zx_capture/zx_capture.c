...
//int zx_buf_gmx_pent_size = sizeof(zx_buf_gmx_sc);
...
//uint8_t* zx_buf_p = zx_buf_gmx_sc;
void zx_copy_pix_gmx_sc(void)
{
  for(int j = 0; j < UVC_VIDEO_HEIGHT; j++) {
    for(int k = 0; k < UVC_VIDEO_WIDTH; k++) {
      //ucv_buf[(UVC_VIDEO_HEIGHT-1)-j][k] = zx_pix_tab[zx_buf_gmx_sc[j+42][k+88]];
      ucv_buf[(UVC_VIDEO_HEIGHT-1)-j][k] = zx_pix_tab[*(((uint8_t*)dcmi_buf)+(j+42)*432+(k+88))];
      //*((uint8_t*)zx_buf_gmx_sc+(j+42)*432*(k+88))
    }
  }
}
