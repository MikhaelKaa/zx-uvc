...
void MX_USB_DEVICE_Init(void)
{
  ...
  if (USBD_Init(&hUsbDeviceHS, &HS_Desc, DEVICE_HS) != USBD_OK) Error_Handler();
  if (USBD_RegisterClass(&hUsbDeviceHS, &USBD_UVC) != USBD_OK)Error_Handler();
  if (USBD_UVC_RegisterCamera(&hUsbDeviceHS, &USBD_UVC_Camera) != USBD_OK)Error_Handler();
  if (USBD_Start(&hUsbDeviceHS) != USBD_OK)Error_Handler();
